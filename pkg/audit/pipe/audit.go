...
const (
	BPF_OBJECT_NAME  = "restricted-pipe"
	BPF_PROGRAM_NAME = "restricted_pipe"
...
func setupBPFProgram() (*libbpfgo.Module, error) {
	bytecode, err := bpf.EmbedFS.ReadFile("bytecode/restricted-pipe.bpf.o")
	if err != nil {
		return nil, err
	}
...
func RunAudit(ctx context.Context, wg *sync.WaitGroup, conf *config.Config) error {
	log.Info("Launching the pipe audit...")
	defer wg.Done()
	...
	<-ctx.Done()
	mgr.Close()
	log.Info("Terminated the pipe audit.")

	return nil
}

func newPipeLog(event auditLog) log.PipeLog {
